...
/**
 * Type guard to check if a tool call is TaskCreate
 */
export function isTaskCreate(toolCall: ToolCall): boolean {
  return toolCall.name === 'TaskCreate'
}

/**
 * Type guard to check if a tool call is TaskUpdate
 */
export function isTaskUpdate(toolCall: ToolCall): boolean {
  return toolCall.name === 'TaskUpdate'
}

/**
 * Type guard to check if a tool call is TaskList
 */
export function isTaskList(toolCall: ToolCall): boolean {
  return toolCall.name === 'TaskList'
}

/**
 * Type guard to check if a tool call is TaskGet
 */
export function isTaskGet(toolCall: ToolCall): boolean {
  return toolCall.name === 'TaskGet'
}
...
import type { AiCliProvider } from './preferences'

/**
 * A delegated task with assigned provider/model
 * Used when splitting a plan across multiple AI providers
 */
export interface DelegatedTask {
  ...
  description: string
  /** Assigned AI provider for this task */
  assignedProvider: AiCliProvider
...
/**
 * User's default delegation preferences
 * Stored in app preferences, applies to all sessions
 */
export interface DelegationPreferences {
  /** Whether multi-model delegation is enabled */
  enabled: boolean
  /** Default provider for new tasks */
...
export interface OrchestrationTask {
  ...
  recommendedProvider: AiCliProvider
