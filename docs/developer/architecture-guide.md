...
│   └── ProjectList / WorktreeList
├── MainWindowContent (Primary content area)
│   ├── SessionBoard (when no worktree selected)
│   └── ChatWindow (when worktree selected)
│       ├── SessionTabBar
│       ├── ChatView / ReviewResultsPanel
│       └── ChatInputArea
├── RightSidebar (Collapsible panel)
│   └── ContextPanel / FileTree
└── Global Overlays
...
│   ├── modals/          # CommitModal, PRModal, MagicModal
│   └── ui/              # Shadcn UI components
├── hooks/               # Custom React hooks
│   ├── useUIStatePersistence.ts
│   ├── useSessionStatePersistence.ts
│   └── useMainWindowEventListeners.ts
├── lib/
│   └── commands/        # Command system implementation
├── services/            # TanStack Query + Tauri integration
├── store/               # Zustand stores (chat-store, ui-store, projects-store)
